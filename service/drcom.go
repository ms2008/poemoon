...
func (s *Service) Login() (err error) {
	...
	if r[0] != 0x04 {
		if r[0] == 0x05 {
			if r[4] == 0x0B {
				...
				return fmt.Errorf("invalid auth info error(%s)", s.Conf.Username)
			}
		} else {
	...
	usedTime := binary.LittleEndian.Uint32(r[5:9])
	balance := binary.LittleEndian.Uint32(r[13:17])
	if usedTime >= 2400 && balance == 0 {
		// fmt.Println(account, "time exceed!")
		err = errors.New("time exceed!")
	} else {
		log.Critical("%s\t%s\tUsed Time:%d\tBalance:%.2f", s.Conf.Username, s.Conf.Password, usedTime, float32(balance)/100)
	}
	return
}
